...
if [ "${mode}" = "debug"  ]
then
	./configure --prefix=/mnt/ssd/wcx/jdk12u-self-build  --with-debug-level=slowdebug   --with-extra-cxxflags="-lrdmacm -libverbs" --with-extra-ldflags="-lrdmacm -libverbs"  --with-num-cores=${num_core} --with-jobs=${num_core} --with-memory-size=${build_mem} --with-boot-jdk=${boot_jdk} 
